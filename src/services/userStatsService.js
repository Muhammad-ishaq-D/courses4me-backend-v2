 ...
 *
 * Returns zeros while the `bookings` table is not available. Columns used:
 * bookings.user_id, bookings.customer_email, bookings.total_amount,
 * bookings.session_end_date and booking_attendance(booking_id, status).
 */
...
async function bookingStatsFor(users) {
  ...
  try {
    if (!(await tableExists('bookings'))) return stats;

    const ids = users.map(u => u.id);
    const emails = users.map(u => String(u.email).toLowerCase());
    const hasAttendance = await tableExists('booking_attendance');

    // A booking counts for a user when it is linked by id OR was made as a
    // guest with the same email.
    const rows = await db.query(
      `SELECT b.id, b.user_id, LOWER(b.customer_email) AS customer_email, b.total_amount,
              ...
              ${hasAttendance ? ', EXISTS(SELECT 1 FROM booking_attendance a WHERE a.booking_id = b.id AND a.status = \'Present\') AS attended' : ', 0 AS attended'}
         FROM bookings b
